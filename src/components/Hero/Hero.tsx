...
import { StaticImageData } from 'next/image'; // Import the type for static images

// WhatsApp Button Component (reusable)
interface WhatsAppButtonProps {
  blue?: boolean; // Optional prop to make the button blue
}
...
interface HeroProps {
  title: string;
  description: string;
  image: StaticImageData; // Image source (imported static image)
}
    ...
    <section className={styles.serviceHeroSection}>
      <div className={styles.serviceHeroContainer}>
        ...
        <div className={styles.serviceHeroImage}>
          ...
          <Image
            src={image} // The image source
            alt={`Sessão de ${title} na Vitta Integrale`} // Alt text for accessibility
            layout="fill" // Image fills the parent container
            objectFit="cover" // Image covers the container, cropping if necessary
